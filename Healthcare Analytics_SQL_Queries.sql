...
#Interview questions
#Write a SQL Query to return the patient ID,Visit date and diagnosis for patients whose latest visit was diagnized as'Hypertension".
#Use Subquery to identify the most recent visit per patient.

Select `patient ID`,`Visit Date`,Diagnosis from  visit_1  
where `visit Date`=(select max(`visit Date`) as max_visit from visit_1) and diagnosis="Hypertension";

 #Top 4 common treatment given for 60 years above patients(treatment,visit,patientid)
 select  t.`Treatment Type`,count(t.`treatment type`) as Treatment_count from patient_1 as p join visit_1 as v on p.patientid=v.`Patient ID` 
 JOIN treatment_1 as t on v.visitid=t.`visit ID` where p.age>=60 group by  t.`Treatment Type` order by treatment_count desc limit 4 ;
